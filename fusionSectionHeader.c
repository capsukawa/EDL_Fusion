...
void calculShOff(ElfFileStruct* elf1,ElfFileStruct* elf2 , int* addTailleSection, int* decalageTotal, int* sectionEnPlus, int *decalageSection) {
	...
	*addTailleSection = 0;				// taille additionnee des sections f2 pas ds f1
	while(i!=nbSection2){	// on cherche dans f2 les 
		...
		decalageSection[i] = decalageSection[i] + elf2->sections[j]->header->sh_size; 
...
void fusionSectionHeader(ElfFileStruct* elf1, ElfFileStruct* elf2, ElfFileStruct* elf) {
	...
	calculShOff(elf1, elf2, &addTailleSection, &decalageTotal, &sectionEnPlus, decalageSection);
