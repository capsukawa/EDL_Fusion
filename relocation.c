...
void initRel(ElfFileStruct* elf, FILE* f) {
	...
	for (i=0;i<elf->header->e_shnum;i++) { //pour chaque section
		...
		if(elf->sections[i]->header->sh_type == SHT_REL) {
			...
			elf->sections[i]->relTab = malloc(sizeof(Elf32_Rel)*elf->sections[i]->nbRel);
			...
			for (j=0;j<elf->sections[i]->nbRel;j++) { //pour chaque entrée dans la table de relocation
				...
				elf->sections[i]->relTab[j]=malloc(sizeof(Elf32_Rel)); //allocation memoire
				if (elf->sections[i]->relTab[j]==NULL) {printf("Echec d'allocation mémoire.\n"); abort();}
				fread(elf->sections[i]->relTab[j], 1, sizeof(Elf32_Rel), f); //lecture dans le fichier et affectation dans la structure

				//endianess
				if (elf->header->e_ident[5]==2) {
					elf->sections[i]->relTab[j]->r_offset = reverse_4(elf->sections[i]->relTab[j]->r_offset);
					elf->sections[i]->relTab[j]->r_info = reverse_4(elf->sections[i]->relTab[j]->r_info);
				}
			}
		}
	}
